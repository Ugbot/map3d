...
const POST_HEIGHT_M = 6;
const PER_TILE_CAP = 4000;
...
export class StreetLightsLayer implements Layer {
  ...
  update(_t: number, sunAltitude: number, glow: number): void {
    // Lights come on as the sun drops. Past dusk they're max emissive; full
    // daytime they're off (we still draw the poles).
    const night = Math.max(0, -sunAltitude);
    this.headMaterial.emissiveIntensity = night * 2.4 * (0.5 + glow * 0.5);
  }
}
